...
model = ChainCRF()
one_slack_svm = OneSlackSSVM(model=model, C=.1, inference_cache=50, tol=0.1, verbose=3)
n_slack_svm = NSlackSSVM(model=model, C=.1, tol=0.1, verbose=3)
bcfw_svm = FrankWolfeSSVM(model=model, C=.1, max_iter=100, tol=0.1, verbose=3, check_dual_every=3)
one_slack_svm.fit(X_train, y_train)
n_slack_svm.fit(X_train, y_train)
bcfw_svm.fit(X_train, y_train)
...
ax[0].plot(n_slack_svm.timestamps_[1:], np.array(n_slack_svm.objective_curve_),
           label="n-slack cutting_plane")
ax[0].plot(n_slack_svm.timestamps_[1:],
           np.array(n_slack_svm.primal_objective_curve_),
...
ax[1].plot(np.where(inference_run)[0],
           np.array(one_slack_svm.primal_objective_curve_)[inference_run],
           label="one-slack primal")
ax[1].plot(np.array(n_slack_svm.objective_curve_),
           label="n-slack cutting_plane")
ax[1].plot(np.array(n_slack_svm.primal_objective_curve_),
           label="n-slack primal")
